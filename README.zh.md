...
`状态: alpha` · `许可证: MIT` · `引擎: 纯 Python 标准库(零依赖)` · `测试: 74 通过`
...
- **不可跳过的收尾** —— `session-check --strict` 不允许会话在缺少"卡或理由、log 行、next_action、
  今日更新的 CONTEXT.md"时结束,记忆才真正留得住(且摘要有上限,不会随着月复一月而上下文过载)。
...
(74 个单元测试 + 一次从零验收,全绿)。只有真实会话能证明的:**教学对话本身的质量**(取决于宿主模型)。

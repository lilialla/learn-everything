...
`状态: alpha` · `许可证: MIT` · `引擎: 纯 Python 标准库(零依赖)` · `测试: 46 通过`
...
- **长文档摄入** —— `scripts/structure.py`(标准库切分器)+ `methods/reading-guide.md`(导读:自顶向下的
  大纲,逐块教学)+ `adapters/doc_ingest/`(OCR/抽取)。
...
(46 个单元测试 + 一次从零验收,全绿)。只有真实会话能证明的:**教学对话本身的质量**(取决于宿主模型)。
